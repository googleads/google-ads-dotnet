...
using static Google.Ads.GoogleAds.V3.Resources.MutateJob.Types;
...
namespace Google.Ads.GoogleAds.Examples.V3
{
    ...
    /// </summary>
    public class AddCompleteCampaignsUsingMutateJob : ExampleBase
    {
        /// <summary>
        ...
        {
            AddCompleteCampaignsUsingMutateJob codeExample =
                new AddCompleteCampaignsUsingMutateJob();
        ...
        public override string Description
        {
            get
            {
                return "This code example adds complete campaigns including campaign budgets, " +
                    "campaigns, ad groups and keywords using MutateJobService.";
            }
        }
        ...
        public void Run(GoogleAdsClient client, long customerId)
        {
            ...
            try
            {
                ...
                AddAllMutateJobOperations(mutateJobService, customerId, mutateJobResourceName);
                Operation<Empty, MutateJobMetadata> operationResponse =
                    RunMutateJob(mutateJobService, mutateJobResourceName);
                PollMutateJob(operationResponse);
                FetchAndPrintResults(mutateJobService, mutateJobResourceName);
            }
        ...
        /// <summary>
        /// Creates the mutate job.
        /// </summary>
        /// <param name="mutateJobService">The mutate job service.</param>
        /// <param name="customerId">The Google Ads customer ID for which the call is made.</param>
        /// <returns>The resource name of the created mutate job.</returns>
        private static string CreateMutateJob(MutateJobServiceClient mutateJobService,
            long customerId)
        {
            string mutateJobResourceName =
                mutateJobService.CreateMutateJob(customerId.ToString()).ResourceName;
            Console.WriteLine($"Created a mutate job with resource name: " +
        ...
        /// <param name="mutateJobService">The mutate job service.</param>
        /// <param name="customerId">The Google Ads customer ID for which the call is made.</param>
        /// <param name="mutateJobResourceName">The resource name of mutate job to which the mutate
        /// job operations will be added.
        /// </param>
        private static void AddAllMutateJobOperations(MutateJobServiceClient mutateJobService,
            long customerId, string mutateJobResourceName)
        {
            AddMutateJobOperationsResponse response =
                mutateJobService.AddMutateJobOperations(
                    new AddMutateJobOperationsRequest()
                    {
                        ResourceName = mutateJobResourceName,
        ...
        /// operations.
        /// </summary>
        /// <param name="mutateJobService">The mutate job service client.</param>
        /// <param name="mutateJobResourceName">The resource name of mutate job to be run.</param>
        /// <returns>The operation response from running mutate job.</returns>
        private Operation<Empty, MutateJobMetadata> RunMutateJob(
            MutateJobServiceClient mutateJobService, string mutateJobResourceName)
        {
            Operation<Empty, MutateJobMetadata> operationResponse =
                mutateJobService.RunMutateJob(mutateJobResourceName);
            Console.WriteLine($"Mutate job with resource name '{mutateJobResourceName}' has been " +
                $"executed.");
        ...
        /// <summary>
        /// Polls the server until the mutate job execution finishes by setting the total
        /// time to wait before time-out.
        /// </summary>
        /// <param name="operationResponse">The operation response used to poll the server.</param>
        private static void PollMutateJob(Operation<Empty, MutateJobMetadata> operationResponse)
        {
        ...
        /// <summary>
        /// Fetches and prints all the results from running the mutate job.
        /// </summary>
        /// <param name="mutateJobService">The mutate job service.</param>
        /// <param name="mutateJobResourceName">The resource name of mutate job to get its results.</param>
        private static void FetchAndPrintResults(MutateJobServiceClient mutateJobService,
            string mutateJobResourceName)
        {
            Console.WriteLine($"Mutate job with resource name '{mutateJobResourceName}' has " +
                $"finished. Now, printing its results...");

            ListMutateJobResultsRequest request = new ListMutateJobResultsRequest()
            {
                ResourceName = mutateJobResourceName,
                PageSize = PAGE_SIZE,
            };
            ListMutateJobResultsResponse resp = new ListMutateJobResultsResponse();
            // Gets all the results from running mutate job and prints their information.
            foreach (MutateJobResult mutateJobResult in
                mutateJobService.ListMutateJobResults(request))
            {
                if (!mutateJobResult.IsFailed)
                {
                    Console.WriteLine($"Mutate job result #{mutateJobResult.OperationIndex} is " +
                        $"successful and response is of type " +
                        $"'{mutateJobResult.MutateOperationResponse.ResponseCase}'.");
                }
                else
                {
                    Console.WriteLine($"Mutate job result #{mutateJobResult.OperationIndex} " +
                        $"failed with error message {mutateJobResult.Status.Message}.");

                    foreach (GoogleAdsError error in mutateJobResult.Failure.Errors)
                    {
        ...
        /// </summary>
        /// <param name="customerId">The Google Ads customer ID for which the call is made.</param>
        /// <returns>The mutate operations to be added to a mutate job.</returns>
        ...
        private static CampaignBudgetOperation BuildCampaignBudgetOperation(long customerId)
        {
            ...
            CampaignBudget budget = new CampaignBudget()
            {
                ...
                Name = "Mutate job Budget #" + ExampleUtilities.GetRandomString(),
            ...
            string campaignBudgetResourceName)
        {
            ...
            for (int i = 0; i < NUMBER_OF_CAMPAIGNS_TO_ADD; i++)
            {
                ...
                Campaign campaign = new Campaign()
                {
                    ...
                    Name = "Mutate job campaign #" + ExampleUtilities.GetRandomString(),
        ...
        /// created some invalid characters to demonstrate how MutateJobService returns information
        /// about such errors.
        /// </summary>
