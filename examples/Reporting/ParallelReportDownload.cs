...
namespace Google.Ads.GoogleAds.Examples.V3
{
    ...
    public class ParallelReportDownload : ExampleBase
    {
        ...
        public static void Main(string[] args)
        {
            ...
            string customerIds = "INSERT_CUSTOMER_ID_1_HERE, INSERT_CUSTOMER_ID_2_HERE";

            // Optional login customer ID if your access to the CIDs is via a manager account.
            string loginCustomerId = "INSERT_MANAGER_ACCOUNT_ID_HERE";
        ...
        public void Run(GoogleAdsClient client, string customerIds, string loginCustomerId = null)
        {
            // If a manager ID is supplied, update the login credentials.
            if (loginCustomerId != null)
            {
                client.Config.LoginCustomerId = loginCustomerId;
            }
            ...
            List<long> ids = customerIds.Split(',').Select(long.Parse).ToList();

            try
            {
                // Begin downloading reports and block program termination until complete.
                Task task = RunDownloadParallelAsync(googleAdsService, ids);
                task.Wait();
            }
        ...
        private async Task RunDownloadParallelAsync(
            GoogleAdsServiceClient googleAdsService, List<long> customerIds)
        {
            ...
            foreach (string query in GAQL_QUERY_STRINGS)
            {
                ...
                {
                    Console.WriteLine($"\tReport {reportId} requested for CID {customerId}.");
            ...
            ConcurrentBag<ReportDownload> responses)
        {
            try
            {
                ...
                return await googleAdsService.SearchStreamAsync(
                    ...
                    {
                        // Store the results.
                        responses.Add(new ReportDownload(customerId, reportId, resp));
                    }
                );
            }
        ...
        {
            private long CustomerId { get; }
            private int ReportId { get; }
            private SearchGoogleAdsStreamResponse Response { get; }

            /// <summary>
            /// Initialize a saved report with a specific CID, an integer identifier, and the
            /// returned response from a request.
            /// </summary>
            /// <param name="customerId">The customer ID for whom the report was requested.</param>
            /// <param name="reportId">Integer to simplify identification of the request.</param>
            /// <param name="response">The data received from a successful request.</param>
            public ReportDownload(long customerId, int reportId,
