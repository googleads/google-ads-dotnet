  ...
  public class GetArtifactMetadata : ExampleBase {
    ...
    public override string Description {
      get {
        return "This code example gets the metadata, such as whether the artifact is " +
            "selectable, filterable and sortable, of an artifact. The artifact can be either " +
            "a resource (such as customer, campaign) or a field (such as metrics.impressions, " +
    ...
    public void Run(GoogleAdsClient client, long customerId, string artifactName) {
      ...
      GoogleAdsFieldServiceClient googleAdsFieldService = client.GetService(
          Services.V0.GoogleAdsFieldService);
      ...
      try {
        ...
        if (googleAdsFields.Any()) {
          ...
          foreach (GoogleAdsField googleAdsField in googleAdsFields) {
            Console.WriteLine("An artifact named '{0}' with category '{1}' and data type '{2}' " +
                "{3} selectable, {4} filterable, {5} sortable and {6} repeated.",
            ...
            List<string> selectableLists = new List<string>(googleAdsField.SelectableWith);
            selectableLists.Sort();

            Console.WriteLine("The artifact can be selected with the following artifacts:");
            foreach (string item in selectableLists) {
              Console.WriteLine("- " + item);
            }
          }
        } else {
          Console.Error.WriteLine($"The specified artifact '{artifactName}' doesn't exist.");
        }
