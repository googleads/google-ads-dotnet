...
{
    /// <summary>
    /// Tests the GetResponsiveSearchAds code example.
    /// </summary>
    [TestFixture]
    [Category("CodeExamples")]
    internal class GetResponsiveSearchAdsTest : ExampleTestsBase
    {
        /// <summary>
        ...
        public void Init()
        {
            ...
            string adGroupAd1ResourceName = ExampleTestsUtils.CreateResponsiveSearchAd(
                client, adGroupResourceName);
            string adGroupAd2ResourceName = ExampleTestsUtils.CreateResponsiveSearchAd(
                client, adGroupResourceName);
        }
        ...
        public void TestRun()
        {
            RunExample(delegate ()
            {
                new GetResponsiveSearchAds().Run(client, config.ClientCustomerId, adGroupId);
            });
        }
        ...
        public void TestRunEmptyAdGroupId()
        {
            RunExample(delegate ()
            {
                new GetResponsiveSearchAds().Run(client, config.ClientCustomerId, null);
            });
        }
    }
}
